  ...
  *_iterateRule(event, rule, window, options) {
  ...
  _weekdayTargets(rule) {
    if (!rule._weekdayTargets) {
      rule._weekdayTargets = rule.byDay
        .map(byDay => {
          const parsed = RRuleParser.parseByDayToken(byDay);
          return parsed ? WEEKDAYS[parsed.weekday] : undefined;
        })
  ...
  getNextMonthly(date, rule, _timezone) {
    ...
    if (rule.byMonthDay && rule.byMonthDay.length > 0) {
      ...
      // Nth weekday of month (e.g., "2nd Tuesday"). Accepts the string
      // format from RRuleParser ("MO", "2TU", "-1FR") and { nth, weekday }.
      const byDay = rule.byDay[0];
      const parsed = RRuleParser.parseByDayToken(byDay);
      if (!parsed) {
        throw new RangeError(this._invalidWeekdayMessage(byDay));
      }

      // Move to the first so the month step cannot overflow from a 29th-31st
      next.setDate(1);
      next.setMonth(next.getMonth() + rule.interval);
      this.setToNthWeekdayOfMonth(next, parsed.weekday, parsed.nth || 1);
   ...
   * @param {string} weekday - Weekday code (e.g., 'MO'; an ordinal prefix is ignored)
   * @param {number} nth - Occurrence within the month (positive from the start, negative from the end)
   * @throws {RangeError} If the weekday code is not one of SU, MO, TU, WE, TH, FR, SA
   */
  setToNthWeekdayOfMonth(date, weekday, nth) {
    // Weekday searches step until getDay() equals the target, so a code
    // that does not name a weekday is refused rather than searched for
    const parsed = RRuleParser.parseByDayToken(weekday);
    if (!parsed) {
      throw new RangeError(this._invalidWeekdayMessage(weekday));
    }
    const targetDay = WEEKDAYS[parsed.weekday];
    date.setDate(1); // Start at first of month

    // Find first occurrence (at most six days away)
   ...
   * @private
   */
  _invalidWeekdayMessage(weekday) {
    const shown =
      weekday && typeof weekday === 'object' ? JSON.stringify(weekday) : String(weekday);
    return `RecurrenceEngineV2: BYDAY value "${shown}" does not name a weekday (SU, MO, TU, WE, TH, FR, SA)`;
  }
